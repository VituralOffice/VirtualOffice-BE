...
import { MockUtils } from "src/utils/tests/mock-utils";
...
describe("ICacheService", () => {
      ...
      providers: [
        {
          ...
          useFactory: (config: { url: "redis:redis:0000" }) =>
            new RedisService(
              config,
              MockUtils.setMock({ log: jest.fn(), warn: jest.fn() })
            ),
        },
      ],
    }).compile();
  ...
  describe("isConnected", () => {
    ...
    test("should isConnected unsuccessfully", async () => {
      ...
      service.client = MockUtils.setMock(mock);
      await expect(service.isConnected()).rejects.toThrow(
        "redis disconnected."
      );
    });
  });
  ...
  describe("set", () => {
    ...
    test("should set unsuccessfully", async () => {
      ...
      service.client = MockUtils.setMock(mock);
      await expect(service.set(KEY_MOCK, "valueMock")).rejects.toThrow(
        `cache set error: ${KEY_MOCK} valueMock`
      );
    });
  });
  ...
  describe("get", () => {
    test("should get successfully", async () => {
      ...
      await expect(service.get("keyMock")).resolves.toEqual({
        key: "keyMock",
        value: "valueMock",
      });
    });
  ...
  describe("setMulti", () => {
    test("should setMulti successfully", async () => {
      ...
      await expect(
        service.setMulti([{ key: "keyMock", value: "mockValue" }])
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe("pExpire", () => {
    ...
    test("should pExpire unsuccessfully", async () => {
      ...
      service.client = MockUtils.setMock(mock);
      await expect(service.pExpire(KEY, 1000)).rejects.toThrow(
        `set expire error key: ${KEY}`
      );
    });
  });
  ...
  describe("hSet", () => {
    test("should hSet successfully", async () => {
      ...
      service.client = MockUtils.setMock(mock);
      await expect(
        service.hSet("keyMock", "fieldMock", "valueMock")
      ).resolves.toEqual(1);
    });
